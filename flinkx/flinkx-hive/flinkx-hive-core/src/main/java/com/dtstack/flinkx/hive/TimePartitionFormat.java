...
public class TimePartitionFormat {
    ...
    private static final long CONSTANT_TWO_MONTH_TIME = 1000 * 60 * 60 * 24 * 30 * 2L;
    private static final long CONSTANT_TWO_YEAR_TIME = 1000 * 60 * 60 * 24 * 30 * 365 * 2L;
    ...
    public static TimePartitionFormat getInstance(String peStr) {
        if (PartitionEnum.YEAR.name().equalsIgnoreCase(peStr)) {
            partitionEnum = PartitionEnum.YEAR;
        } else if (PartitionEnum.MONTH.name().equalsIgnoreCase(peStr)) {
            partitionEnum = PartitionEnum.MONTH;
        } else if (PartitionEnum.DAY.name().equalsIgnoreCase(peStr)) {
    ...
    public String currentTime() {
        if (PartitionEnum.YEAR == partitionEnum) {
            return DateUtil.getYearFormatter().format(new Date());
        } else if (PartitionEnum.MONTH == partitionEnum) {
            return DateUtil.getMonthFormatter().format(new Date());
        } else if (PartitionEnum.DAY == partitionEnum) {
    ...
    public boolean isTimeout(long lastWriteTime) {
        if (null == partitionEnum) {
            return false;
        } else if (TimePartitionFormat.PartitionEnum.YEAR == partitionEnum) {
            return (System.currentTimeMillis() - lastWriteTime) >= CONSTANT_TWO_YEAR_TIME;
        } else if (TimePartitionFormat.PartitionEnum.MONTH == partitionEnum) {
            return (System.currentTimeMillis() - lastWriteTime) >= CONSTANT_TWO_MONTH_TIME;
