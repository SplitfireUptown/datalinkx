...
public class HiveUtil {
    ...
    private static final String ANALYZE_PARTITION_TEMPLATE = "analyze table %s partition (%s) compute statistics";
    ...
    public final static String TABLE_COLUMN_Origin_KEY = "originKey";
     ...
     * 创建hive的分区
     */
    public void analyzePartition(String tablePath, String partition) {
        if(StringUtils.isEmpty(tablePath) || StringUtils.isEmpty(partition)) return;

        Connection connection = null;
        try {
            connection = HiveDbUtil.getConnection(connectionInfo);
            String sql = String.format(ANALYZE_PARTITION_TEMPLATE, tablePath, partition);
            logger.info("analyzePartition sql: {}", sql);
            HiveDbUtil.executeSqlWithoutResultSet(connectionInfo, connection, sql);
        } catch (Exception e) {
            logger.error("", e);
            throw e;
