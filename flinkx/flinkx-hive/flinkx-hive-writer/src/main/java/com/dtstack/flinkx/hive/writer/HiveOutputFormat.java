...
public class HiveOutputFormat extends BaseRichOutputFormat {
    ...
    public FormatState getFormatState() {
        ...
        //重新统计分区中的数据信息到表信息中
//        hiveUtil.analyzePartition(tbPath, partitionPath);
        return formatState;
    }
    ...
    public void writeRecord(Row row) throws IOException {
        ...
        if (row.getField(0) instanceof Map) {
            ...
            tbPath = PathConverterUtil.regaxByRules(event, tableBasePath, distributeTableMapping);
            fromLogData = true;
        } else {
        ...
        try {
            formatPair = getHdfsOutputFormat(tbPath, event);
        } catch (Exception e) {
        ...
        Row rowData = row;
        if (fromLogData) {
            rowData = setChannelInformation(event, row.getField(1), formatPair.getSecond().getoriginColumns().isEmpty() ? formatPair.getSecond().getColumns() :formatPair.getSecond().getoriginColumns());
        }
        try {
    ...
    private Pair<BaseHdfsOutputFormat, TableInfo> getHdfsOutputFormat(String tablePath, Map event) throws Exception {
        ...
        partitionPath = String.format(HiveUtil.PARTITION_TEMPLATE, partition, partitionValue);
