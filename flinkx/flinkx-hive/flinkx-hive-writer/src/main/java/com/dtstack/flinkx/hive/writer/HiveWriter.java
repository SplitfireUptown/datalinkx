        ...
        if (StringUtils.isNotEmpty(tablesColumn)) {
            ...
            for (Map.Entry<String, List<Map<String, Object>>> entry : tableColumnMap.entrySet()) {
                ...
                for (Map<String, Object> column : tableColumns) {
                    tableInfo.addColumnAndTypeAndOriginName(MapUtils.getString(column, HiveUtil.TABLE_COLUMN_KEY), HiveUtil.getHiveColumnType(MapUtils.getString(column, HiveUtil.TABLE_COLUMN_TYPE)), MapUtils.getString(column, HiveUtil.TABLE_COLUMN_Origin_KEY));
                }
