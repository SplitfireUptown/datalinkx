            ...
            for (int pos = 0; pos < row.getArity(); pos++) {
                ...
                if(obj != null) {
                    if(CollectionUtils.isNotEmpty(columnTypeList)) {
                        String columnType = columnTypeList.get(pos).toLowerCase(Locale.ROOT);
                        obj = typeConverter.convert(obj, columnType);
                        if("tinyint".equalsIgnoreCase(columnType) || "bit".equalsIgnoreCase(columnType)) {
                            if(obj instanceof Boolean) {
                                obj = ((Boolean) obj ? 1 : 0);
                            }
                        } else if (Arrays.asList("boolean").contains(columnType)) {
                            if(obj instanceof Boolean) {
                                obj = ((Boolean) obj ? "t" : "f");
                            }
                        }
