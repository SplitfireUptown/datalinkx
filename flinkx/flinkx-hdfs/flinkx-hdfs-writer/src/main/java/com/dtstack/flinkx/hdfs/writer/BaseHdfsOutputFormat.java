...
public abstract class BaseHdfsOutputFormat extends BaseFileOutputFormat {
    ...
    /** minio 连接配置*/
    protected Map<String,Object> minioConfig;
   ...
   //如果key为string类型的值是map 或者 list 会使用gson转为json格式存入
    ...
    protected void checkOutputDir() {
        try{
            ...
            if(fs.exists(dir)){
                if(!OVERWRITE_MODE.equalsIgnoreCase(writeMode) && fs.isFile(dir)){
    ...
    protected void openSource() throws IOException{
        try{
            ...
            minioClient = MinioUtil.getMinioClient(minioConfig);
        } catch (Exception e){
            throw new RuntimeException("Get FileSystem/MinioClient error", e);
        }
    }
    ...
    protected void moveTemporaryDataFileToDirectory() throws IOException{
        ...
        for(FileStatus dataFile : dataFiles) {
            ...
            LOG.info("moveTemporaryDataFileToDirectory Rename temp file:{} to dir:{}", dataFile.getPath(), dir);
        }
    }
    ...
    protected void moveAllTemporaryDataFileToDirectory() throws IOException {
        ...
        for(FileStatus dataFile : dataFiles) {
            fs.rename(dataFile.getPath(), dir);
            LOG.info("moveAllTemporaryDataFileToDirectory Rename temp file:{} to dir:{}", dataFile.getPath(), dir);
        }
    }
