...
import com.dtstack.flinkx.config.ErrorLimitConfig;
...
public abstract class BaseRichOutputFormat extends org.apache.flink.api.common.io.RichOutputFormat<Row> implements CleanupWhenUnsuccessful {
    ...
    protected void writeBinaryMultipleRecords(List<Row> data) {
        LOG.warn("binary write, size: {}", data.size());
        if (data.size() == 1) {
            this.writeSingleRecord(data.get(0));
        } else {
            int m = data.size() / 2;
            LOG.warn("binary write into: 0 - {}", m);
            try {
                rows = data.subList(0, m);
                writeMultipleRecords();
            } catch (Exception e) {
                writeBinaryMultipleRecords(rows);
            }

            LOG.warn("binary write into: {} - {}", m, data.size());
            try {
                rows =  data.subList(m, data.size());
                writeMultipleRecords();
            } catch (Exception e) {
                writeBinaryMultipleRecords(rows);
            }
    ...
    protected void writeRecordInternal() {
        try {
            ...
            if(restoreConfig.isRestore()) {
                ...
                if (ErrorLimitConfig.ERROR_TRY_PLAN_BINARY.equals(errorTryPlan)) {  // 二分法重试
                    writeBinaryMultipleRecords(rows);
                } else if (ErrorLimitConfig.ERROR_TRY_PLAN_ONE_BY_ONE.equals(errorTryPlan)) {   // 逐条重试
    ...
    public void writeRecord(Row row) throws IOException {
        ...
        long bytesLen = row.toString().getBytes().length;
        long startTs = System.currentTimeMillis();
        long writeTs = 0;
        if(batchInterval <= 1) {
            writeSingleRecord(internalRow);
            writeTs = System.currentTimeMillis() - startTs;
        } else {
        ...
        try{
            ...
            try{
                ...
                try {
                    ...
                    LOG.error("checkErrorLimit Exception", e);
                    throw e;
                } finally {
