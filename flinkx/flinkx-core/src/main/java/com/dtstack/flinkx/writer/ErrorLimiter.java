...
//                    + "]\n" + errorDataStr + errMsg);
            if (maxErrors > 0 && errors > maxErrors || maxErrors == 0 && errorData != null) {
                throw new RuntimeException("WritingRecordError: error writing record [" + errors + "] exceed limit [" + maxErrors
                        + "]\n\n" + GsonUtil.GSON.toJson(errorData) + "\n\n" + errorDataStr + errMsg);
            }
        }
