...
public class AccumulatorCollector {
    ...
    private static final int MAX_COLLECT_ERROR_TIMES = 2;
        ...
        for (String monitorUrl : monitorUrls) {
            try {
                ...
                if (null != map) {
                    ...
                    if (null != userTaskAccumulators && userTaskAccumulators.size() > 0) {
                    ...
                    LOG.info(String.format("monitorUrl: %s; response: %s", monitorUrl, response));
                }
            } catch (Exception e){
                checkErrorTimes();
    ...
    private void checkErrorTimes() {
        ...
        if (collectErrorTimes > MAX_COLLECT_ERROR_TIMES){
            ...
            close();
            LOG.error("更新统计数据出错次数超过最大限制100次，为了确保数据正确性，任务自动失败");
