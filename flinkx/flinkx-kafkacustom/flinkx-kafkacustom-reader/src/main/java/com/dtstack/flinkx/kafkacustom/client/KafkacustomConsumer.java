 ...
 * @author tudou
 */
public class KafkacustomConsumer extends KafkaBaseConsumer {
    public KafkacustomConsumer(Properties properties) {
        super(properties);
    }
    ...
    public KafkaBaseConsumer createClient(String topic, String group, KafkaBaseInputFormat format, KafkaInputSplit kafkaInputSplit) {
        ...
        client = new KafkacustomClient(clientProps, 100L, format, kafkaInputSplit);
        return this;
    }
    ...
    public void submitOffsets(Collection<kafkaState> kafkaStates){
        if(client != null){
            KafkacustomClient kafkacustomClient = (KafkacustomClient) this.client;
            kafkacustomClient.submitOffsets(kafkaStates);
        }
    }
}
