...
public class HbaseInputFormat extends BaseRichInputFormat {
            ...
            try {
                ...
                if (StringUtils.isNotEmpty(columnValue)) {
                    ...
                    if (KEY_ROW_KEY.equals(columnName)) {
                        ...
                        if(arr == null){
                            arr = new byte[2][];
                            String[] arr1 = columnName.split(":", 2);
                    ...
                    if ("date".equalsIgnoreCase(columnType) && col != null && col instanceof Date) {
                        col = ft.format((Date)col);
                    }
                }
                row.setField(i, col);
            } catch(Exception e) {
    ...
    public static FilterList formatFilter(String filter) throws IOException {
        JsonNode jsonNode = new ObjectMapper().readTree(filter);
        Map<String, CompareFilter.CompareOp> hashMap = new HashMap<>();
        String type = jsonNode.get("type").asText();
        hashMap.put(">", CompareFilter.CompareOp.GREATER);
        hashMap.put(">=", CompareFilter.CompareOp.GREATER_OR_EQUAL);
        hashMap.put("<", CompareFilter.CompareOp.LESS);
        hashMap.put("<=", CompareFilter.CompareOp.LESS_OR_EQUAL);
        hashMap.put("=", CompareFilter.CompareOp.EQUAL);
        hashMap.put("!=", CompareFilter.CompareOp.NOT_EQUAL);
        hashMap.put("like", CompareFilter.CompareOp.EQUAL);
        hashMap.put("not like", CompareFilter.CompareOp.NOT_EQUAL);
        hashMap.put("first", CompareFilter.CompareOp.EQUAL);
        FilterList filterByJsonNode = getFilterByJsonNode(jsonNode.get("value"), hashMap, type);
        return filterByJsonNode;
    }

    private static FilterList getFilterByJsonNode(JsonNode jsonNode, Map<String, CompareFilter.CompareOp> hashMap, String type) {
        HashMap<String, FilterList.Operator> map = new HashMap<>();
        map.put("and", FilterList.Operator.MUST_PASS_ALL);
        map.put("or", FilterList.Operator.MUST_PASS_ONE);

        List<Filter> filters = new ArrayList<>();
        for (JsonNode node : jsonNode) {
            if (!node.get("type").asText().equals("compare")) {
                filters.add(getFilterByJsonNode(node.get("value"), hashMap, node.get("type").asText()));
            } else {
                JsonNode field = node.get("value").findValue("field");
                if (KEY_ROW_KEY.equals(field.asText())) {
                    JsonNode op = node.get("value").findValue("op");
                    JsonNode value = node.get("value").findValue("value");
                    RowFilter rowFilter = null;
                    if (op.asText().equals("like") || op.asText().equals("not like")) {
                        rowFilter = new RowFilter(hashMap.get(op.asText()),
                                new SubstringComparator(value.asText()));
                    } else if (op.asText().equals("first")) {
                        rowFilter = new RowFilter(hashMap.get(op.asText()),
                                new BinaryPrefixComparator(Bytes.toBytes(value.asText())));
                    } else {
                        rowFilter = new RowFilter(hashMap.get(op.asText()),
                                new BinaryComparator(Bytes.toBytes(value.asText())));
                    }
                    filters.add(rowFilter);
                } else {
                    String family = field.asText().substring(0, field.toString().indexOf(":") - 1);
                    String column = field.asText().substring(field.toString().indexOf(":"));
                    JsonNode op = node.get("value").findValue("op");
                    JsonNode value = node.get("value").findValue("value");
                    SingleColumnValueFilter singleColumnValueFilter = null;
                    if (op.asText().equals("like") || op.asText().equals("not like")) {
                        singleColumnValueFilter = new SingleColumnValueFilter(Bytes.toBytes(family), Bytes.toBytes(column),
                                hashMap.get(op.asText()), new SubstringComparator(value.asText()));
                    } else if (op.asText().equals("first")) {
                        singleColumnValueFilter = new SingleColumnValueFilter(Bytes.toBytes(family), Bytes.toBytes(column),
                                hashMap.get(op.asText()), new BinaryPrefixComparator(Bytes.toBytes(value.asText())));
                    } else {
                        singleColumnValueFilter = new SingleColumnValueFilter(Bytes.toBytes(family), Bytes.toBytes(column),
                                hashMap.get(op.asText()), Bytes.toBytes(value.asText()));
                    }
        ...
        return new FilterList(map.get(type), filters);
    }
